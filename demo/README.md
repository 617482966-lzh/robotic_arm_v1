...
末端世界坐标增量运动也采用相同的六位规则，但六位依次对应
X、Y、Z、Rx、Ry、Rz：X=`0x01`、Y=`0x02`、Z=`0x04`、Rx=`0x08`、
Ry=`0x10`、Rz=`0x20`。程序只为非零增量置位，例如X、Z和Rz同时运动时
自动得到 `ckStatus=0x25`。无需在界面或调用代码中手工填写掩码。

正式界面的关节速度范围为1～20 deg/s。连接后固定全局速度10%，正式控制层
根据J1～J6各自的实机标定曲线，将输入角速度转换成action4.speed。单轴运动使用
对应关节的专属曲线；多轴运动因action4只有一个公共speed，程序选择保证所有活动轴
均不超过输入速度的安全百分比。
...
| 参数 | 含义 |
|---|---|
| `tool_x` | 保持末端姿态，沿当前工具坐标系+X轴的世界三维方向运动 |
| `distance_mm` | 三维空间距离；正值沿正向，负值沿反向 |
| `--speed` | action51物理线速度，单位mm/s |

## 4. 方向计算

贯入试验固定沿世界坐标系Z负方向运动，与U/V/W姿态角无关：
