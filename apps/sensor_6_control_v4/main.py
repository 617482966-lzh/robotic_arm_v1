...
from main_window_4 import MainWindow, SHOVEL_POINTS
...
WINDOWS_APP_USER_MODEL_ID = "JLU.RoboticArm.ControlSystem.V4"
...
class AppController:
    ...
    def _begin_test(self, kind, target, limit, direction=None):
        ...
        self._active_test = {
            ...
            "accumulated_j6": 0.0,
            # 运动命令确认发送前只允许采样，不允许阈值逻辑抢先提交
            # safe_stop。否则高预载荷会让高优先级停止清空尚未执行的剪切命令。
            "motion_armed": False,
        }
        self._sample_active_test()
        self._sample_timer.start()
    ...
    def _sample_active_test(self):
        ...
        if (
            state.get("motion_armed", False)
            and (reached or overloaded)
    ...
    def _on_data_ready(self, fx, fy, fz, tx, ty, tz):
        ...
        if (
            state
            and state.get("motion_armed", False)
            and not state.get("stop_requested")
        ):
            if state["kind"] == "testDisp":
    ...
    def _on_robot_command_done(self, command):
        state = self._active_test
        expected_command = {
            "testDisp": "move_along_tool_x",
            "testShear": "joint_increment",
        }.get(state["kind"] if state else None)
        if state and command == expected_command:
            state["motion_armed"] = True
            state["motion_command_confirmed_at"] = time.monotonic()
