...
该模块独立实现 V4 图表、试验、位置记忆、连接状态和末端/关节控制，
...
SETTINGS_PATH = os.path.join(APPLICATION_DIR, "sensor_6_control_v4.ini")
UI_PATH = os.path.join(RESOURCE_DIR, "main_window_4.ui")
...
QPushButton#robotBtnConnect[connected="true"], QPushButton#sensorBtnConnect[connected="true"] { background-color: #6b2a2a; border-color: #dc3545; }
QPushButton#robotBtnConnect[connected="true"]:hover, QPushButton#sensorBtnConnect[connected="true"]:hover { background-color: #8b2020; }
...
QGroupBox#sensorForceGroup {
    margin-top: 12px;
    padding-top: 2px;
}
QPushButton#robotBtnConnect,
QPushButton#sensorBtnConnect,
QPushButton#sensorRefreshBtn {
    min-height: 24px;
    max-height: 24px;
    padding: 2px 8px;
}
...
QPushButton#robotBtnReset_zengliang,
QPushButton#robotBtnReset_global,
QPushButton#robotJointBtnReset_zengliang,
QPushButton#robotJointBtnReset_global,
QPushButton#testDispReset,
QPushButton#testShearReset,
QPushButton#testShovelReset {
    background-color: #78620b;
    border-color: #d1aa00;
    color: #fff5bd;
    font-weight: bold;
}
QPushButton#robotBtnReset_zengliang:hover,
QPushButton#robotBtnReset_global:hover,
QPushButton#robotJointBtnReset_zengliang:hover,
QPushButton#robotJointBtnReset_global:hover,
QPushButton#testDispReset:hover,
QPushButton#testShearReset:hover,
QPushButton#testShovelReset:hover {
...
QPushButton#robotBtnConnect[connected="true"], QPushButton#sensorBtnConnect[connected="true"] { background-color: #b64242; border-color: #962f2f; color: white; }
...
QPushButton#robotBtnConnect, QPushButton#sensorBtnConnect, QPushButton#sensorRefreshBtn { min-height: 24px; max-height: 24px; padding: 2px 8px; }
QPushButton#robotBtnReset_zengliang, QPushButton#robotBtnReset_global,
QPushButton#robotJointBtnReset_zengliang, QPushButton#robotJointBtnReset_global,
QPushButton#testDispReset, QPushButton#testShearReset, QPushButton#testShovelReset { background-color: #e0b51b; border-color: #b38c00; color: #302600; font-weight: bold; }
QLabel#poseValue_X, QLabel#poseValue_Y, QLabel#poseValue_Z,
QLabel#poseValue_Rx, QLabel#poseValue_Ry, QLabel#poseValue_Rz,
QLabel#jointValue_J1, QLabel#jointValue_J2, QLabel#jointValue_J3,
...
QGroupBox#sensorForceGroup { margin-top: 12px; padding-top: 2px; }
...
class MainWindow(QMainWindow):
    """加载 ``main_window_4.ui`` 的六维传感器 V4 主窗口。"""
    ...
    def _setup_ui_from_file(self):
        ...
        self.setWindowTitle("机械臂控制与六维力传感数据采集系统 - V4")
    ...
    def _connect_common_signals(self):
        """绑定 V4 使用的连接、试验、图表和公共动作控件。"""
        direct_bindings = (
            ("robotBtnConnect", self._toggle_robot_connection),
            ("sensorBtnConnect", self._toggle_sensor_connection),
    ...
    def _connect_signals(self):
        ...
        reset_bindings = (
            ("robotBtnReset_zengliang", "robotTarget{}_zengliang", WORLD_AXES),
            ("robotBtnReset_global", "robotTarget{}_global", WORLD_AXES),
            (
                "robotJointBtnReset_zengliang",
                "robotJointTarget{}_zengliang",
                JOINT_AXES,
            ),
            (
                "robotJointBtnReset_global",
                "robotJointTarget{}_global",
                JOINT_AXES,
            ),
        )
        for button_name, input_template, axes in reset_bindings:
            button = self._find_child(button_name)
            if button:
                button.clicked.connect(
                    lambda _checked=False, template=input_template, names=axes:
                    self._reset_motion_inputs(template, names)
                )
    ...
    def _configure_control_geometry(self):
        ...
        if connection_group:
            connection_group.setGeometry(4, 4, 511, 64)
            connection_line = self._find_child("robotConnLine")
            if connection_line:
                connection_line.setGeometry(10, 20, 491, 32)

        # 控制区按紧凑节奏排列；手动几何用于保证不同 Qt/Windows 缩放下不漂移。
        ...
        if pose_group:
            pose_group.setGeometry(4, 73, 511, 300)
            current_xyz = self._find_child("poseCurrentRowXYZ")
            current_rotation = self._find_child("poseCurrentRowRotation")
            if current_xyz:
                current_xyz.setGeometry(10, 14, 491, 29)
            if current_rotation:
                current_rotation.setGeometry(10, 43, 491, 29)
            ...
            row_y = {
                "zengliang": {"X": 76, "Y": 76, "Z": 76,
                               "Rx": 111, "Ry": 111, "Rz": 111},
                "global": {"X": 181, "Y": 181, "Z": 181,
                            "Rx": 216, "Ry": 216, "Rz": 216},
            }
            for suffix in ("zengliang", "global"):
                ...
                speed_y = 146 if suffix == "zengliang" else 251
                ...
                for widget, geometry in (
                    (speed_label, (9, speed_y, 94, 32)),
                    (speed_slider, (106, speed_y + 6, 116, 20)),
                    (speed_spin, (226, speed_y + 1, 64, 29)),
                ):
                    if widget:
                        ...
                        widget.show()
                if speed_slider:
                    speed_slider.setFixedSize(116, 20)
                if speed_spin:
                    speed_spin.setFixedSize(64, 29)
                send_button = self._find_child(f"robotBtnPTP_{suffix}")
                if send_button:
                    send_button.setParent(pose_group)
                    send_button.setGeometry(294, speed_y, 100, 32)
                    send_button.setFixedSize(100, 32)
                    send_button.show()
                reset_button = self._find_child(f"robotBtnReset_{suffix}")
                if reset_button:
                    reset_button.setParent(pose_group)
                    reset_button.setGeometry(398, speed_y, 100, 32)
                    reset_button.setFixedSize(100, 32)
                    reset_button.show()
        ...
        if joint_group:
            joint_group.setGeometry(4, 378, 511, 315)
        if memory_group:
            memory_group.setGeometry(4, 698, 511, 285)
        if action_container:
            action_container.setGeometry(10, 988, 495, 46)
        scroll_widget = self._find_child("robotScrollWidget")
        if scroll_widget:
            scroll_widget.setMinimumHeight(1041)

        sensor_connection_group = self._find_child("sensorConnGroup")
        if sensor_connection_group and sensor_connection_group.layout():
            sensor_connection_group.setFixedHeight(62)
            sensor_connection_group.layout().setContentsMargins(8, 0, 8, 6)
            sensor_connection_group.layout().setSpacing(4)

        sensor_force_group = self._find_child("sensorForceGroup")
        if sensor_force_group and sensor_force_group.layout():
            sensor_force_group.layout().setContentsMargins(8, 0, 8, 6)
            sensor_force_group.layout().setSpacing(3)
            sensor_force_group.layout().setAlignment(Qt.AlignmentFlag.AlignTop)

        ip_edit = self._find_child("robotIpEdit")
        port_edit = self._find_child("robotPortEdit")
        if ip_edit:
            ip_edit.setFixedSize(145, 30)
        if port_edit:
            port_edit.setFixedSize(62, 30)
        robot_connection_button = self._find_child("robotBtnConnect")
        if robot_connection_button:
            robot_connection_button.setFixedSize(78, 30)
        ...
        sensor_port = self._find_child("sensorComPort")
        sensor_refresh = self._find_child("sensorRefreshBtn")
        sensor_connection_button = self._find_child("sensorBtnConnect")
        sensor_status = self._find_child("sensorStatusLabel")
        if sensor_port:
            sensor_port.setFixedSize(110, 30)
        if sensor_refresh:
            sensor_refresh.setFixedSize(60, 30)
        if sensor_connection_button:
            sensor_connection_button.setFixedSize(68, 30)
        if sensor_status:
            sensor_status.setFixedWidth(48)
            sensor_status.setAlignment(
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
            )
        ...
        if joint_group and joint_group.layout():
            joint_group.layout().setContentsMargins(8, 0, 8, 6)
            joint_group.layout().setSpacing(2)
        ...
        for suffix in ("zengliang", "global"):
            joint_slider = self._find_child(f"robotJointSpeedSlider_{suffix}")
            joint_spin = self._find_child(f"robotJointSpeedSpin_{suffix}")
            joint_send = self._find_child(f"robotJointBtnPTP_{suffix}")
            joint_reset = self._find_child(f"robotJointBtnReset_{suffix}")
            if joint_slider:
                joint_slider.setMinimumWidth(116)
            if joint_spin:
                joint_spin.setFixedWidth(64)
            if joint_send:
                joint_send.setFixedSize(100, 32)
            if joint_reset:
                joint_reset.setFixedSize(100, 32)

        memory_type_label = self._find_child("memoryTypeLabel")
        memory_type_hint = self._find_child("memoryTypeHint")
        if memory_type_label:
            memory_type_label.setFixedWidth(72)
        if memory_type_hint:
            memory_type_hint.setFixedWidth(112)

        if memory_group and memory_group.layout():
            memory_group.layout().setContentsMargins(8, 0, 8, 6)
            memory_group.layout().setSpacing(2)
        ...
        for index in range(1, 6):
            ...
            slot_label = self._find_child(f"memLabel_{index}")
            if slot_label:
                slot_label.setFixedWidth(28)
                slot_label.setAlignment(
                    Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter
                )
        ...
        # QUiLoader 会缓存堆叠页在 Designer 中的旧最小宽度。控件宽度调整后
        # 主动重算，否则右栏虽然只有约410 px，第三个“重置”按钮仍会被裁掉。
        for owner_name in (
            "testStandardPage",
            "testShovelPage",
            "testStackedWidget",
            ...
            owner = self._find_child(owner_name)
            if owner is None:
                continue
    ...
    def _reset_motion_inputs(self, input_template, axes):
        """只归零当前一组输入，不发送任何机械臂运动指令。"""
        for axis in axes:
            input_widget = self._find_child(input_template.format(axis))
            if input_widget is not None:
                input_widget.setValue(0.0)
        self.statusBar().showMessage("当前控制组的六个输入值已归零", 2500)
    ...
    def _toggle_robot_connection(self):
        """同一个按钮根据真实连接状态执行连接或断开。"""
        if self._robot_connected:
            self._on_robot_disconnect()
            return
        button = self._find_child("robotBtnConnect")
        if button:
            button.setText("连接中…")
            button.setEnabled(False)
        self._on_robot_connect()
    ...
    def _toggle_sensor_connection(self):
        """同一个按钮根据真实连接状态执行连接或断开。"""
        if self._sensor_connected:
            self._on_sensor_disconnect()
            return
        button = self._find_child("sensorBtnConnect")
        if button:
            button.setText("连接中…")
            button.setEnabled(False)
        self._on_sensor_connect()
    ...
    def _set_robot_connected(self, state):
        state = bool(state)
        self._robot_connected = state
        button = self._find_child("robotBtnConnect")
        if button:
            button.setText("断开" if state else "连接")
            button.setProperty("connected", state)
    ...
    def _refresh_memory_label(self, index):
        ...
        formatted = [self._format_memory_number(v) for v in memory.values]
        values = ", ".join(formatted)
        label.setText(
            f"{prefix} [{', '.join(formatted[:3])},\n{', '.join(formatted[3:])}]"
        )
        label.setToolTip(
